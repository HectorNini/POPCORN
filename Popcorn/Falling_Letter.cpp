...
AFalling_Letter::AFalling_Letter(EBrick_Type brick_type, ELetter_Type letter_type, int x, int y)
    : Letter_Type(letter_type), X(x), Y(y),
    Brick_Type(brick_type), Rotation_Step(2),
    Next_Rotation_Tick(AsConfig::Current_Timer_Tick + Ticks_Per_Step),
    Falling_Letter_State(EFLS_Normal)
{
    Letter_Cell.left = X;
    Letter_Cell.top = Y;
...
{
    int letters_popularity = AsConfig::Rand(All_Letters_Popularity);
...
{//Вывод падающей буквы
    ...
    else
    {
        ...
        //Настройка матрицы поворота буквы
