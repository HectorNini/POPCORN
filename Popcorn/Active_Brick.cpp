...
{
    if (Fade_Step >= Max_Fade_Step ) //!!! раньше здесь бы мат  -1
        return true;
    else
        return false;
}
...
AActive_Brick_Unbreakable::AActive_Brick_Unbreakable(int level_x, int level_y)
    :AActive_Brick(EBT_Unbreakable, level_x, level_y), Region(0), Animation_Step(0)
{
...
AAdvertisement::AAdvertisement(int level_x, int level_y, int width, int height)
    :Deformation_Ratio(0.0),Falling_Speed(0.0), Acceleration_Step(0.2), Ball_Y_Offset(0), Ball_X(0), Ball_Y(0), Ball_Width(Ball_Size * AsConfig::Global_Scale), Ball_Height(Ball_Size* AsConfig::Global_Scale), Empty_Region(0), Level_X(level_x), Level_Y(level_y), Width(width), Height(height), Brick_Regions(0)
{
    ...
    Ball_X = Ad_Rect.left + 9 * AsConfig::Global_Scale + Ball_Width / 2 + 1;
    Ball_Y = Ad_Rect.top + 2 * AsConfig::Global_Scale + Ball_Height / 2;
...
{
   //1. Заказ перерисовки над фрагр=ментами, не закрытыми кирпичами
    RECT rect;
    ...
    //2. Смещение шарика рекламы
    Falling_Speed += Acceleration_Step;
    Ball_Y_Offset = High_Ball_Treshold - (int)pow(Falling_Speed, 2);
...
void AAdvertisement::Draw(HDC hdc, RECT& paint_area) 
{
    ...
    // 1. Рамка
    // 1.1 Тонкая синяя со скругленными краями
    ...
    // 3. Тень
    // 3.1 Размер 8х6, пока шарик полностью над столом
    ...
    Arc(hdc, 
        ...
        x + 4*scale, y + scale, 
        x + scale, y + 4*scale);
    // 5.3 Летает вверх/вниз
    // 5.4 Сплющивается до 16х9
...
{ //Вывод неактивного кирпича на уровне
    ...
    int size = (Circle_Size - 1) * AsConfig::Global_Scale - 1;
    ...
    {
        AsConfig::Red_Color.Select(hdc);
        Ellipse(hdc, x, y, x + Circle_Size * AsConfig::Global_Scale - 1, y + Circle_Size * AsConfig::Global_Scale - 1);
        AsConfig::White_Color.Select(hdc);
        Arc(hdc,
...
{ //Вывод неактивного кирпича на уровне
    ...
    //1. Рисуем внутренние прямоугольники
...
{//Рисование внутреннего прямоугольника
    const int scale = AsConfig::Global_Scale;
    RECT stage_rect;
...
AActive_Brick_Teleport::AActive_Brick_Teleport(int level_x, int level_y, ABall *ball, AActive_Brick_Teleport *destination_teleport)
    :AActive_Brick(EBT_Teleport, level_x, level_y), Animation_Step(0), Ball(0), Destination_Teleport(destination_teleport), Teleport_State(ETS_Starting)
{
    Set_Ball(ball);
}
//---------------------------------------------------------------------------------------------------------
